...
For a major release, one should perform these checks for each of the batch files `gap.bat` and `gapcmd.bat` in the 64-bit version, and also check the 32-bit version built with and without readline support. For a minor release, one could perform more thorough checks with `gap.bat` and just check that the two other files also start GAP.
...
Additionally:
* check that packages with compiled kernel modules work (Browse, cvec, edim, io, orb and others) 
...
* Prepare new data for links in all manuals: in the `dev` directory of the main GAP repository, call `gap -r -A LinksOfAllHelpSections.g` and then move the produced `AllLinksOfAllHelpSections.data` file into the `lib` directory of the GAP website repository (this will use Browse package to open a dialog window - press `q` to continue).
...
```
mv *.exe ../www.gap-system.org/pub/gap/gap-4.11/exe/
mv *.tar.bz2 ../www.gap-system.org/pub/gap/gap-4.11/tar.bz2/
mv *.tar.gz ../www.gap-system.org/pub/gap/gap-4.11/tar.gz/
mv *-win.zip ../www.gap-system.org/pub/gap/gap-4.11/win.zip/
mv *-win64.zip ../www.gap-system.org/pub/gap/gap-4.11/win.zip/
mv *-core.zip ../www.gap-system.org/pub/gap/gap4core/
mv *.zip ../www.gap-system.org/pub/gap/gap-4.11/zip/
```

Check that everything (locations, permissions etc.) look right with
```
ls -la ../www.gap-system.org/pub/gap/gap-4.11/*/gap-4.X.Y* 
```
...
* Update corresponding release notes on [Wiki](https://github.com/gap-system/gap/wiki/) to collect changes for the next GAP release
...
* Maintainers of alternative installers/distributions may now update them:
  ...
  - Gap.app: <https://cocoagap.sourceforge.io/>
